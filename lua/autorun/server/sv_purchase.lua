...
util.AddNetworkString( "vs.admin" )
...
local function shield_Buy( p, id )
	...
	do -- How many shields user can buy
		local bToAdd = id
		local maxBRank = vs.cfg.Vip.Steam[ p:SteamID64() ] or vs.cfg.Vip.Rank[ p:GetUserGroup() ] or vs.cfg.Shield.Max

		if bToAdd + p.sAmount > maxBRank then
			local overShields = ( p.sAmount + bData.amount ) - maxBRank
			bToAdd = bToAdd - overShields
...
net.Receive( "vs.admin" , function( _, p )
	if ( !p:IsAdmin() ) then return end -- don't want any exploit now do we
	local e = net.ReadEntity()
	local g = net.ReadInt( 32 )
	e.sAmount = e.sAmount + g
	e:SetNWInt( "shields", e.sAmount )
	print( tostring( p ) .. " Gave " .. tostring( e ) .. " " .. g .. " shield(s)" .. "\n" )
end )